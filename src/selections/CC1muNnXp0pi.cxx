...
void CC1muNnXp0pi::ComputeTrueObservables(AnalysisEvent* Event) {
  ...
  for ( size_t d = 0u; d < num_mc_daughters; ++d ) {
	...
	double mom = real_sqrt( std::pow(energy, 2) - std::pow(NEUTRON_MASS, 2) );
	  ...
	  mc_lead_neutron_px_ = v3n.X();
          mc_lead_neutron_py_ = v3n.Y();
...
bool CC1muNnXp0pi::Selection(AnalysisEvent* Event) {
  ...
  for ( int p = 0; p < Event->num_pf_particles_; ++p ) {
    ...
    if ( p == muon_candidate_idx_ ) {
      ...
      float muon_cand_px = sel_muon_candidate_px[p]*muon_mom;
      float muon_cand_py = sel_muon_candidate_py[p]*muon_mom;
      float muon_cand_pz = sel_muon_candidate_pz[p]*muon_mom;
      ...
      float track_length = Event->track_length_->at( p );
      // Bad tracks in the searchingfornues TTree can have bogus track lengths. This skips those.
	  ...
	  // Compute the squared distances between the candidate and the start/end points
      ...
      if ( track_length > 0. && end_contained){
        ...
        v3p = v3p.Unit() * p_mom;
        if(p_mom > 0.25 && p_mom < 1.0 && sec_p_cand_vtx_disp < 10.0){
  ...
  if(num_proton_candidates > 0u){
    ...
    float range_mom_lead_p = real_sqrt( lead_p_KE*lead_p_KE + 2.*PROTON_MASS*lead_p_KE );
    ...
    event_calo_energy_leading_p += std::sqrt(range_mom_lead_p*range_mom_lead_p+PROTON_MASS*PROTON_MASS) - PROTON_MASS + .0309; //Proton KE + Proton binding energy to argon
  }
  ...
  float neutron_cand_trans_position_mag = 0;
  float neutron_cand_position_mag = 0;
  ...
  if (num_secondary_proton_cands == 1)
  {
    ...
    neutron_cand_trans_position_mag = sqrt(pow(neutron_cand_position.X(),2)+pow(neutron_cand_position.Y(),2));
    neutron_cand_position_mag = std::sqrt(pow(neutron_cand_position.X(),2)+pow(neutron_cand_position.Y(),2)+pow(neutron_cand_position.Z(),2));
...
int CC1muNnXp0pi::CategorizeEvent(AnalysisEvent* Event) {
  ...
  if ( IsEventMCSignal() ) { //CCnumu >=1 neutrons, 0 pions
      if(sig_num_neutron_above_100MeV == 1){ // 1 neutron
	if(sig_nProtons_in_Momentum_range == 0){return kNuMuCC0p0pi1n; } //0 protons
	else if (sig_nProtons_in_Momentum_range == 1) {return kNuMuCC1p0pi1n; } //1 proton
	else {return kNuMuCCNp0pi1n;} //N protons
      }
      else if(sig_num_neutron_above_100MeV > 1){ //N neutrons
        if(sig_nProtons_in_Momentum_range == 0){return kNuMuCC0p0piNn;} //0 protons
        else if (sig_nProtons_in_Momentum_range == 1) {return kNuMuCC1p0piNn; } //1 proton
        else{return kNuMuCCNp0piNn;} //N protons
      }
  }
  else if (!sig_mc_no_fs_pi0_ || !sig_mc_no_charged_pi_above_threshold_) { // 1>= pions
    return kNuMuCCNpi;
  }
  else if (sig_num_neutron_above_100MeV == 0){ //CCnumu 0 neutrons
