...
CC1mu1p0piXn::CC1mu1p0piXn() : SelectionBase("CC1mu1p0piXn") {
  CalcType = kOpt1;
  fPP = new TF1("fPP","29.354172*x-14.674918",0.3,0.5);
}

void CC1mu1p0piXn::DefineConstants() {
  DefineTrueFV(10.,246.,-105.,105.,10.,1026.);
  DefineRecoFV(10.,246.,-105.,105.,10.,1026.);
}

void CC1mu1p0piXn::ComputeRecoObservables(AnalysisEvent* Event) {
...
void CC1mu1p0piXn::ComputeTrueObservables(AnalysisEvent* Event) {
...
int CC1mu1p0piXn::CategorizeEvent(AnalysisEvent* Event) {
  ...
  //Burke edits are adding the if else statement about neutron presence and adding neutron label to each category
  if ( Is_CC1muNp0pi_Event ) { //CCnumu >= 1 proton, 0 pion
    if ( sig_num_neutron_above_100MeV == 0 ) { //0 neutrons
      if (sig_mc_n_threshold_proton == 1) { return kNuMuCC1p0pi0n; } // 1 proton 
      else { return kNuMuCCNp0pi0n; } // N protons
    }
    else if ( sig_num_neutron_above_100MeV == 1 ) { //1 neutron
      if (sig_mc_n_threshold_proton == 1) { return kNuMuCC1p0pi1n; } // 1 proton
      else { return kNuMuCCNp0pi1n; } // N protons
    }
    else{ // N neutrons
      if (sig_mc_n_threshold_proton == 1) { return kNuMuCC1p0piNn; } // 1 proton
      else { return kNuMuCCNp0piNn; } // N protons
    }
  }
  else if (!sig_no_pions_) { // N pions
    return kNuMuCCNpi;
  }
  else if (sig_num_neutron_above_100MeV == 0 && sig_mc_n_threshold_proton == 0) {return kNuMuCC0p0pi0n;} //O neutron 0 protons
  else if (sig_num_neutron_above_100MeV == 1 && sig_mc_n_threshold_proton == 0) {return kNuMuCC0p0pi1n;} //1 neutron 0 protons
  else if (sig_num_neutron_above_100MeV > 1 && sig_mc_n_threshold_proton == 0) {return kNuMuCC0p0piNn;} //N neutrons 0 protons
  else {return kNuMuCCOther;}
}

bool CC1mu1p0piXn::DefineSignal(AnalysisEvent* Event) {
  //==============================================================================================================================
  ...
  for ( size_t p = 0u; p < Event->mc_nu_daughter_pdg_->size(); ++p ) {
    ...
    //Begin Burke Edits
    else if ( pdg == NEUTRON && real_sqrt( std::pow(ParticleMomentum,2) + std::pow(NEUTRON_MASS*NEUTRON_MASS,2) ) > 0.1 ) { sig_num_neutron_above_100MeV++; }
    //End Burke Edits
...
bool CC1mu1p0piXn::Selection(AnalysisEvent* Event) {
  //==============================================================================================================================
  ...
  //Begin Burke Edits
  TVector2 event_transverse_momentum(0.0, 0.0);
  //End Burke Edits
  ...
  if (sel_ntrack_eq_2_) {
    ...
    float Muon_dirx = Event->track_dirx_->at( CandidateMuonIndex );
    float Muon_diry = Event->track_diry_->at( CandidateMuonIndex );
    float Muon_dirz = Event->track_dirz_->at( CandidateMuonIndex );
    ...
    float Proton_dirx = Event->track_dirx_->at( CandidateProtonIndex );
    float Proton_diry = Event->track_diry_->at( CandidateProtonIndex );
    float Proton_dirz = Event->track_dirz_->at( CandidateProtonIndex );
    ...
    TVector3 Muon_direction(Muon_dirx, Muon_diry, Muon_dirz);
    TVector3 Proton_direction(Proton_dirx, Proton_diry, Proton_dirz);

    event_transverse_momentum.SetX(Muon_direction.Unit().X() * CandidateMuMom + Proton_direction.X() * CandidatePMom);
    event_transverse_momentum.SetY(Muon_direction.Unit().Y() * CandidateMuMom + Proton_direction.Y() * CandidatePMom);
    //Save missing momentum for the event
    event_missing_trans_px = -1*event_transverse_momentum.X();
    event_missing_trans_py = -1*event_transverse_momentum.Y();
...
void CC1mu1p0piXn::DefineOutputBranches() {
...
void CC1mu1p0piXn::DefineCategoryMap() {
