...
void STV_Tools::CalculateSTVs(TLorentzVector p4Muon, std::vector<TLorentzVector> p4Hadron, STVCalcType CalcOpt) {
  ...
  TVector3 MuonVectorTrans(p4Muon.Px(), p4Muon.Py(), 0.);
  double MuonVectorTransMag = MuonVectorTrans.Mag();
  
  TVector3 MuonVectorLong(0., 0., p4Muon.Pz());
  double MuonVectorLongMag = MuonVectorLong.Mag();

  TLorentzVector p4TotalHadron;
  double HadronKE = 0;

  for(unsigned int i = 0u; i < p4Hadron.size(); i++){
    p4TotalHadron+=p4Hadron[i];
     if(fabs(p4Hadron[i].M() - PROTON_MASS) < 1e-2){  // FIXME: I use the invariant mass of particle to separate proton and pion, need to find a better way to do pid
       HadronKE += p4Hadron[i].E() - PROTON_MASS;
     }
     else if(fabs(p4Hadron[i].M() - PI_PLUS_MASS) < 1e-2){
       HadronKE += p4Hadron[i].E();
     }
  }

  TVector3 HadronVectorTrans(p4TotalHadron.Px(), p4TotalHadron.Py(), 0.);
  double HadronVectorTransMag = HadronVectorTrans.Mag();
  
  TVector3 HadronVectorLong(0.,0.,p4TotalHadron.Pz());
  double HadronVectorLongMag = HadronVectorLong.Mag();
  
  //TLorentzVector ProtonLorentzVector(ProtonVector,ProtonEnergy);
  

  TVector3 PtVector = MuonVectorTrans + HadronVectorTrans;
  fPt = PtVector.Mag();

  TVector2 fPt_2DVec = (p4Muon + p4TotalHadron).Vect().XYvector();
  ...
  fDeltaPhiT = TMath::ACos( (- MuonVectorTrans * HadronVectorTrans) / ( MuonVectorTransMag * HadronVectorTransMag ) ) * 180./TMath::Pi();
  if (fDeltaPhiT > 180.) { fDeltaPhiT -= 180.; }
  if (fDeltaPhiT < 0.) { fDeltaPhiT += 180.; }
  ...
  fECal = p4Muon.E() + HadronKE + BindingEnergy_GeV; // GeV
  
  // QE Energy Reconstruction
  // it is the energy reconstruction used by cherenkov detectors
  // where they cannot reconstruct any protons and instead they use only the lepton kinematics
  // MicroBooNE will not use this variable.
...
//  fEQE = EQENum / EQEDen;
  ...
  TLorentzVector nuLorentzVector(0.,0.,fECal,fECal);
  TLorentzVector qLorentzVector = nuLorentzVector - p4Muon;
  fQ2 = - qLorentzVector.Mag2();
  ...
  TVector2 xTUnit = zUnit.Cross(p4Muon.Vect()).XYvector().Unit();
  fPtx = xTUnit.X()*fPt_2DVec.X() + xTUnit.Y()*fPt_2DVec.Y();

  TVector2 yTUnit = (-p4Muon.Vect()).XYvector().Unit();
  fPty= yTUnit.X()*fPt_2DVec.X() + yTUnit.Y()*fPt_2DVec.Y();
  
  // -------------------------------------------------------------------------------------------------------------------------
  
  // JLab Light Cone Variables
  // ignore it, it is something that my advisor asked me to check with the jlab people ---  from Afro
  
  fEMiss = 0;
  fPMiss = 0;
  fkMiss = 0;
  fA = 0;
//  TLorentzVector MissLorentzVector = p4Muon + p4TotalHadron - nuLorentzVector;
...
//  fPMiss = (MissLorentzVector.Vect()).Mag();
  ...
  // Suggestion from Jackson to avoid Ecal assumption
//  fPMissMinus = (p4Muon.E() - p4Muon.Pz()) + (p4TotalHadron.E() - p4TotalHadron.Pz());
...
//  fECalMB = MuonEnergy + HadronKE + BindingEnergy_GeV; // GeV, after discussion with Andy F who got the numbers from Jan S
//  TLorentzVector nuLorentzVectorMB(0.,0.,fECalMB,fECalMB);
//  TLorentzVector qLorentzVectorMB = nuLorentzVectorMB - p4Muon;
  ...
  //  fPL = 0.5 * R - (MAPrime * MAPrime + fPt * fPt) / (2 * R);
    ...
    break;
  case kOpt4:
    // MicroBooNE calculate PL in that way
    // https://journals.aps.org/prd/pdf/10.1103/PhysRevD.109.092007
    fECalMB = p4Muon.E() + HadronKE + BindingEnergy_GeV;
  ...
  fDeltaAlpha3DMu = TMath::ACos( -(p4Muon.Vect() * PnVector) / ( p4Muon.Vect().Mag() * fPn ) ) * 180./TMath::Pi();	
  if (fDeltaAlpha3DMu > 180.) { fDeltaAlpha3DMu -= 180.; }
  if (fDeltaAlpha3DMu < 0.) { fDeltaAlpha3DMu += 180.; }
  
  fDeltaPhi3D = TMath::ACos( (qVector * p4TotalHadron.Vect()) / ( qMag * p4TotalHadron.Vect().Mag() ) ) * 180./TMath::Pi();
  if (fDeltaPhi3D > 180.) { fDeltaPhi3D -= 180.; }
  if (fDeltaPhi3D < 0.) { fDeltaPhi3D += 180.; }	
