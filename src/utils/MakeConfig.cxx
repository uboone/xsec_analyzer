    ...
    const std::string& mc_event_weight)
{
  ...
  std::cout << "FilePropertiesManager instance created\n";
  ...
  if ( show_bin_plots ) {
    ...
    for ( size_t b = 1u; b < bin_low_edges.size(); ++b ) {
      ...
      std::cout<< "true_hist_plot: " << ("true events in " + variable_title + " reco bin "
           + std::to_string(b) + "; " + variable_title + "; events").c_str() << '\n';
  ...
  std::cout << "smear_expr: " << smear_expr << '\n';
